...
func Test_RunnerClosterManager(t *testing.T) {
	...
	t.Run("runner with no tasks or closers should return nil", func(t *testing.T) {
		...
		assert.NoError(t, NewRunnerCloserManager(0).Run(context.Background()))
	})
	...
	t.Run("runner with a task that completes should return nil", func(t *testing.T) {
		...
		var i atomic.Int32
		assert.NoError(t, NewRunnerCloserManager(0, func(context.Context) error {
			i.Add(1)
			return nil
	...
	t.Run("runner with a task and closer that completes should return nil", func(t *testing.T) {
		...
		var i atomic.Int32
		mngr := NewRunnerCloserManager(0, func(context.Context) error {
			i.Add(1)
			return nil
		})
	...
	t.Run("a manger started twice should error", func(t *testing.T) {
		...
		var i atomic.Int32
		m := NewRunnerCloserManager(0, func(ctx context.Context) error {
			i.Add(1)
			return nil
		})
...
func TestClose(t *testing.T) {
	...
	t.Run("calling close should stop the main runner and call all closers", func(t *testing.T) {
		...
		mngr := NewRunnerCloserManager(0, func(ctx context.Context) error {
			close(runnerWaiting)
			<-ctx.Done()
			i.Add(1)
			return nil
...
func TestAddCloser(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			err := NewRunnerCloserManager(0).AddCloser(test.closers...)
